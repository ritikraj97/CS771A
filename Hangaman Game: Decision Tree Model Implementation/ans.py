...
class Node:
	...
	def chose_root(self, all_words, my_words_idx):
	
		ch_ind = -1
		m_len = 0
		for ind in my_words_idx:
			word_len = len(all_words[ind])
			if(word_len > m_len):
				m_len = word_len
				ch_ind = ind
	...
	def chose_ind(self,all_words,my_words_idx):
	...
	def process_node( self, all_words, my_words_idx, history, verbose ):
		...
		if len( history ) == 0:
			# query_idx = -1
			query_idx = self.chose_root(all_words, my_words_idx)
			query = ""
		else:
			# query_idx = np.random.randint( 0, len( all_words ) )
			query_idx = self.chose_ind(all_words,my_words_idx)
